...
import { type WatchOptions, type WatchStopHandle, instanceWatch } from './apiWatch'
import {
  EMPTY_OBJ,
  type IfAny,
  NOOP,
...
> = {
  ...
  $watch<T extends string | ((...args: any) => any)>(
    source: T,
    cb: T extends (...args: any) => infer R ? (...args: [R, R]) => any : (...args: any) => any,
...
export const PublicInstanceProxyHandlers: ProxyHandler<any> = {
  ...
  set({ _: instance }: ComponentRenderContext, key: string, value: any): boolean {
    ...
    if (hasSetupBinding(setupState, key)) {
      ...
      return true
    } else if (__DEV__ && setupState.__isScriptSetup && hasOwn(setupState, key)) {
  ...
  has(
    { _: { data, setupState, accessCache, ctx, appContext, propsOptions } }: ComponentRenderContext,
    key: string,
  ) {
    let normalizedProps
    return (
  ...
  defineProperty(target: ComponentRenderContext, key: string, descriptor: PropertyDescriptor) {
...
// dev only
export function exposePropsOnRenderContext(instance: ComponentInternalInstance) {
  const {
    ctx,
...
// dev only
export function exposeSetupStateOnRenderContext(instance: ComponentInternalInstance) {
