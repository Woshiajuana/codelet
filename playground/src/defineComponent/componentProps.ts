import { TriggerOpTypes, shallowReactive, shallowReadonly, toRaw, trigger } from '@vue/reactivity'
import {
  EMPTY_ARR,
  EMPTY_OBJ,
  type IfAny,
  ...
  if (
    ...
    if (attrs !== rawCurrentProps) {
      for (const key in attrs) {
        if (
          !rawProps ||
          (!hasOwn(rawProps, key) && (!__COMPAT__ || !hasOwn(rawProps, key + 'Native')))
        ) {
          delete attrs[key]
          hasAttrsChanged = true
        }
...
function getTypeIndex(type: Prop<any>, expectedTypes: PropType<any> | void | null | true): number {
